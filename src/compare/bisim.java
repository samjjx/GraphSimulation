...
//		}
	...
	public grpattr bisimulation(cg_graph graph, HashMap<String, String> attr, HashMap<String, Integer> rankv){
		...
		int count =0;
		for(HashSet<String> it:partition){
			System.out.println("rankvalue "+count);
			for(String v:it){
...
//			collapse.add(newhyn);
		...
		for(HashSet<String> ttt:blockMap.keySet()){
			for(String tt:ttt){
				System.out.print(tt+" ");
			}
			System.out.println(" ->" + blockMap.get(ttt));
		}
		/**
...
//		String path = "/disk/scratch/dataset/p2p-Gnutella08.grp";
		String pathgrp = "15-Ran-62586-147892.grp";
		String pathattr = "15-Ran-62586-147892.atr";
		...
		for(String v:Graph.vertexSet()){
			System.out.println(v+" "+attr.get(v));
		}
		for(edge e:Graph.edgeSet()){
			System.out.println("src: "+e.getSource()+ " dst :" +e.getTarget());
		}
		for(String n:Graph.vertexSet()){
			Vector<String> pset = Graph.getParents(n);
			bs.pMap.put(n, pset);
		...
		System.out.println("Print Gscc info");
		Gscc.graphInfo();
		System.out.println("End Gscc info");
